...
function ot(t, n, e, i) {
	if (t[2] && i) {
		...
		if (typeof c == 'object') {
			...
			for (let l = 0; l < o; l += 1) f[l] = n.dirty[l] | c[l];
			return f;
		}
		return n.dirty | c;
	}
...
function W(t) {
	...
	for (let r = 0; r < n.length; r++) {
		const u = n[r].claim_order,
			a = (c > 0 && n[e[c]].claim_order <= u ? c + 1 : L(1, c, (y) => n[e[y]].claim_order, u)) - 1;
		i[r] = e[a] + 1;
		const s = a + 1;
	...
	for (let r = e[c] + 1; r != 0; r = i[r - 1]) {
		for (f.push(n[r - 1]); l >= r; l--) o.push(n[l]);
		l--;
	}
	for (; l >= 0; l--) o.push(n[l]);
	f.reverse(), o.sort((r, u) => r.claim_order - u.claim_order);
	for (let r = 0, u = 0; r < o.length; r++) {
		for (; u < f.length && o[r].claim_order >= f[u].claim_order; ) u++;
	...
	return P(
		...
		(c) => {
			...
			for (let o = 0; o < c.attributes.length; o++) {
				...
				e[l.name] || f.push(l.name);
			}
			f.forEach((o) => c.removeAttribute(o));
		},
		() => i(n)
	);
}
...
function nt(t, n, e, i) {
	const { fragment: c, on_mount: f, on_destroy: o, after_update: l } = t.$$;
	c && c.m(n, e),
		i ||
			j(() => {
				const r = f.map(T).filter(H);
...
function St(t, n, e, i, c, f, o, l = [-1]) {
	...
	if (
		((u.ctx = e
			? e(t, n.props || {}, (s, y, ...N) => {
					...
					return (
						u.ctx &&
							c(u.ctx[s], (u.ctx[s] = S)) &&
							(!u.skip_bound && u.bound[s] && u.bound[s](S), a && it(t, s)),
						y
					);
			  })
			: []),
		...
		if (n.hydrate) {
			G();
			const s = U(n.target);
			u.fragment && u.fragment.l(s), s.forEach(Q);
